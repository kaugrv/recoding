...
// draw the quadrangle :
      ...
      if (s==2) {
         stroke(20*i,20*(i+j),20*j); // color gradient

      }
      if (s==3) {
        stroke(20*i,20*i,20*(i+j)); // color gradient
...
function DrawMesh(w,h) {
 ...
 for (let j=0; j<h/2; j++) {
      for (let i=w-1; i>=0; i--) {
        ijquad_amount=50-DistIJ(i,j)/10; // increasing from right to left (-i), keep the number increasing from row to row (w*j), as in the original artwork (*2)
   ...
   for (let j=0; j<h/2; j++) {
      for (let i=w-1; i>=0; i--) {
        ijquad_amount=50-DistIJ(w-1-i, h-1-j)/10; // split in 2 the previous amount (/2) to draw half brown and half black
