...
function DrawMesh(w,h) {
  ...
  for (let j=0; j<h/2; j++) {
    for (let i=w-1; i>=0; i--) {
      ijquad_amount=2*((w-i-1+w*j)+1); // increasing from right to left (-i), keep the number increasing from row to row (w*j)
   ...
   for (let j=0; j<h/2; j++) {
      for (let i=w-1; i>=0; i--) {
        ...
        // 3 last lines (brown)
        stroke("#8f746d");
        DrawSquare(ijquad_amount, w-1-i, h-1-j); // positions = start bottom right ("rotate")
